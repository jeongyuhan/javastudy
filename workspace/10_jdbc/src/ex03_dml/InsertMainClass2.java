...
public class InsertMainClass2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 변수 처리(?에 값을 전달하기)
			// set타입() 메소드를 이용하여 (n번째?, m값)을 전달해주는 방식으로 처리한다.
			ps.setInt(1, 2); // 1번째 ?에 데이터 "2" 전달
			ps.setString(2, "emily"); // 2번째 ?에 데이터 "emily" 전달
			ps.setString(3, "인사"); // 3번째 ?에 데이터 "인사" 전달
			ps.setDate(4, new Date(System.currentTimeMillis())); // 4번째 ?에 데이터 new Date(System.currentTimeMillis()) 전달
			int result = ps.executeUpdate();
			if (result == 0) {
				System.out.println("행이 추가되지 않았습니다.");
			} else {
				System.out.println("행이 정상적으로 추가되었습니다.");
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
				...
				// close()의 순서는 먼저 생성한 것을 나중에 닫고 나중에 생성된 것을 먼저 닫는 순서를 지켜준다.
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
